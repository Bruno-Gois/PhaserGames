...
function Main(game){
	...
	this.create = function(){
		...
		for (var i=0;i<2;i++){  //limitar numero de marcadores
		...
		explodes = game.add.group(); //efeito de explosao
		explodes.enableBody = true;

		bores = game.add.group();//gerador de maquinas inimigas
	...
	this.update = function(){
		...
		if(!isOver){
			...
			//this.mapTilePlay(); // animacao de agua
			this.enemyMake();
		}
	};
	...
	this.enemyMove = function(enemy){
		if(game.time.now>=enemy.timeToMove){
			...
			go = go>3 ? go-3 : go; // Reduzir a chance de subir
			...
			if(Math.random() < 0.5){  // Disparo aleatório...
	...
	this.fireHit = function(tank, fire){
		...
		fire.kill();
		// Aqui você pode aumentar o julgamento de saúde e, em seguida, decidir se deseja matar.
		this.explodePlay(tank.x+8,tank.y+8,"explode2","boom1");
		tank.kill();
		if(player.alive){ // Como esse método é compartilhado, primeiro determine quem está morto.
			score--;
	...
	this.tileHit = function(fire, tile){
		...
		if([11,12,25,26].indexOf(tile.index) > -1){  //Hit Boss
	...
	this.mapTilePlay = function(){
		if(game.time.now>=map.playTimer){
			map.replace(map.playIndex + 7,(map.playIndex+1)%4 + 7);//Tem um certo impacto no desempenho (talvez haja outras maneiras de alcançar)
	...
	this.actKeyDown = function(){
		if(isOver){
			if(player.alive){ //Vivo? Próximo nível
